...
class RecargaScreen extends StatelessWidget {
  ...
  Future<void> recargar(BuildContext context) async {
    ...
    final res = await Supabase.instance.client.from('billetera').select('saldo').eq('id_usuario', user.id).single();
    double nuevoSaldo = (res['saldo'] as num).toDouble() + monto;

    // 2. Actualizar billetera
    await Supabase.instance.client.from('billetera').update({'saldo': nuevoSaldo}).eq('id_usuario', user.id);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            TextField(controller: _montoController, decoration: const InputDecoration(labelText: "Monto a recargar")),
            const SizedBox(height: 20),
            ElevatedButton(onPressed: () => recargar(context), child: const Text("Confirmar Recarga")),
          ],
        ),
      ),
    );
